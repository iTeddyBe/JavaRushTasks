...
public class Solution {
    ...
    public static boolean isWeekend(LocalDateTime dateTime) {
        ...
        int tmpHour = dateTime.getHour();
        int tmpMins = dateTime.getMinute();

        if (dayOfWeek == SATURDAY) {
            return true;
        }
        if (dayOfWeek == FRIDAY && tmpHour >= WEEKEND_START_FRIDAY_CUT_OFF_HOUR && tmpMins >= 0) {
            return true;
        }
        if (dayOfWeek == SUNDAY && tmpHour < WEEKEND_END_SUNDAY_CUT_OFF_HOUR) {
            return true;
        }
