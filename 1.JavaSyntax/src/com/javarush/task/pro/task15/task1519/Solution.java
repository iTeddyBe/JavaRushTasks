package com.javarush.task.pro.task15.task1519;

import java.io.File;
import java.io.IOException;
import java.nio.file.DirectoryStream;
import java.nio.file.Files;
import java.nio.file.Path;
import java.util.Scanner;

/* 
Поверхностное копирование
*/

public class Solution {
    public static void main(String[] args) throws IOException {
        Scanner scanner = new Scanner(System.in);
        //C:\01\
        //C:\02\
        Path sourceDirectory = Path.of(scanner.nextLine());
        Path targetDirectory = Path.of(scanner.nextLine());

        try (DirectoryStream<Path> listFiles = Files.newDirectoryStream(sourceDirectory)) {
            for (Path path : listFiles) {
                if (Files.isRegularFile(path)) {
                    Path fileName = path.getFileName();
                    //System.out.println(targetDirectory.toString() + "\\" + fileName.toString());
                    String strPathTarget = targetDirectory.toString() + File.separator + fileName.toString();
                    //System.out.println(strPathTarget);
                    Path pathTarget = Path.of(strPathTarget);
                    Path pathResolve = targetDirectory.resolve(path.getFileName());
                    //if (Files.notExists(pathTarget)) {
                    Files.copy(path, pathResolve);
                    //}
                }
            }
        } catch (Exception e) {
            System.out.println("Have error : " + e);
        }

    }
}

